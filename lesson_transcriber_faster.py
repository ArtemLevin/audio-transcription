...
ContentFilterMode = Literal["off", "safe", "medium", "aggressive"]
...
ORGANIZATIONAL_PATTERNS = [
    r"\bменя\s+слышно\b[^.!?\n]{0,80}[.!?]?",
    r"\b(?:видно|видите)\s+(?:экран|демонстрацию|доску|файл|задачу)?\b[^.!?\n]{0,100}[.!?]?",
    r"\bсейчас\s+(?:я\s+)?(?:всё|все)?\s*(?:подключу|включу|открою|загружу|подгружу)\b[^.!?\n]{0,120}[.!?]?",
    r"\bсекундочку\b[^.!?\n]{0,100}[.!?]?",
    r"\bподгружается\b[^.!?\n]{0,100}[.!?]?",
    r"\bоткройте\s+(?:тетрадь|учебник|файл|задание)\b[^.!?\n]{0,120}[.!?]?",
    r"\bздравствуйте\b[.!?]?",
    r"\bдобрый\s+(?:день|вечер|утро)\b[.!?]?",
]

PEDAGOGICALLY_IMPORTANT_KEYWORDS = [
    "задача",
    "условие",
    "решение",
    "ответ",
...
def ensure_ffmpeg_tools() -> None:
    if shutil.which("ffmpeg") is None:
        raise RuntimeError("ffmpeg не найден в PATH. Он нужен только если не указан --skip-audio-prepare.")
    if shutil.which("ffprobe") is None:
        raise RuntimeError("ffprobe не найден в PATH. Обычно он устанавливается вместе с ffmpeg.")
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        for line in lines:
            ...
            if not text:
                continue
            f.write(f"{index}\n")
            f.write(f"{format_srt_timestamp(line.start)} --> {format_srt_timestamp(line.end)}\n")
            f.write(text + "\n\n")
            index += 1
...
def write_timestamped_txt(lines: list[TranscriptLine], output_file: Path) -> None:
    with open(output_file, "w", encoding="utf-8") as f:
        for line in lines:
            text = line.text.strip()
            if text:
                f.write(f"[{format_plain_timestamp(line.start)} — {format_plain_timestamp(line.end)}] {text}\n")
...
def prepare_audio_for_whisper(input_file: Path, output_wav: Path, audio_filter: AudioFilterMode) -> None:
    cmd = [
        "ffmpeg",
        "-y",
        "-threads",
...
def transcribe_audio_file(
    ...
    try:
        ...
        raise RuntimeError(f"faster-whisper не смог начать транскрибацию: {exc}") from exc
        ...
        for segment in segments_generator:
            ...
            lines.append(
                TranscriptLine(
                    ...
                    text=text,
                    avg_logprob=to_optional_float(getattr(segment, "avg_logprob", None)),
                    no_speech_prob=to_optional_float(getattr(segment, "no_speech_prob", None)),
                    compression_ratio=to_optional_float(getattr(segment, "compression_ratio", None)),
                )
            )
    except Exception as exc:
        raise RuntimeError(f"faster-whisper прервал транскрибацию: {exc}") from exc
    ...
    return full_text, lines, info_dict
...
def remove_known_asr_noise_blocks(text: str) -> str:
    replacements = [
        (r"(?:Я не знаю, что это такое[.!?…,\s]*){2,}", "[технический шум автотранскрибации удалён] "),
        (r"(?:Спасибо[.!?…,\s]*){5,}", "[длинный повтор слова «спасибо» удалён] "),
        (r"(?:Повторяйте[.!?…,\s]*){3,}", "[технический повтор фразы «повторяйте» удалён] "),
    ]
...
def collapse_target_repetitions(text: str, min_repeats: int) -> str:
    for phrase in REPEATABLE_PHRASES:
        ...
        text = re.sub(pattern, f"[повтор фразы «{phrase}» удалён] ", text, flags=re.IGNORECASE)
    return text


def collapse_repeated_single_words(text: str, min_repeats: int) -> str:
    pattern = re.compile(
        rf"\b([А-Яа-яЁёA-Za-z0-9_-]{{2,}})\b(?:[,.!?…\s]+\1\b){{{min_repeats - 1},}}",
        flags=re.IGNORECASE,
    )
    return pattern.sub(lambda match: f"[повтор слова «{match.group(1)}» удалён]", text)
...
def remove_aggressive_filler_sentences(text: str) -> str:
    ...
    for sentence in split_sentences(text):
        ...
        if stripped and not FILLER_SENTENCE_PATTERN.match(stripped):
    ...
    for signal in IMPORTANT_STUDENT_SIGNALS:
        ...
        for match in pattern.finditer(text):
            ...
            if key in seen:
                continue
            seen.add(key)
            signals.append({"signal": signal, "position": match.start(), "snippet": snippet})
    return signals


def is_lightweight_sentence(sentence: str) -> bool:
    normalized = sentence.strip().lower()
    normalized = re.sub(r"[^\wа-яё\s-]", "", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"\s+", " ", normalized).strip()
    if not normalized:
        return True
    words = normalized.split()
    return len(words) <= 3 and all(word in CONTENT_LIGHTWEIGHT_PHRASES for word in words)


def contains_pedagogically_important_content(sentence: str) -> bool:
    lowered = sentence.lower()
    if re.search(r"\d", sentence):
        return True
    if any(symbol in sentence for symbol in ["=", ">", "<", "≥", "≤", "+", "-", "\\", "^", "√"]):
        return True
    return any(keyword in lowered for keyword in PEDAGOGICALLY_IMPORTANT_KEYWORDS)


def remove_organizational_fragments(text: str) -> str:
    result = text
...
def build_content_only_transcript(cleaned_text: str, mode: ContentFilterMode) -> str:
    if mode == "off":
        return cleaned_text

    text = remove_organizational_fragments(cleaned_text)
    ...
    for sentence in sentences:
        ...
        important = contains_pedagogically_important_content(stripped)
        lightweight = is_lightweight_sentence(stripped)
        ...
        if mode == "safe":
            ...
            if lightweight:
                continue
            if len(stripped) <= 18 and not important:
                continue
            kept.append(stripped)
            ...
            raise ValueError(f"Неизвестный режим content-filter: {mode}")
...
def clean_transcript_text(raw_text: str, mode: CleanMode) -> dict[str, object]:
    ...
    return {"normalized": normalized, "cleaned": cleaned, "signals": signals}
...
def create_llm_prompt(
    ...
    for item in signals[:30]:
        signal_lines.append(f"- `{item['signal']}`: {item['snippet']}")
    signal_block = "\n".join(signal_lines) if signal_lines else "Явные сигналы затруднений ученика автоматически не найдены."

    if len(transcript_text) <= include_text_limit:
...
Я дам тебе очищенный транскрипт занятия. Для LLM используется режим смысловой фильтрации: `{content_filter_mode}`.

Твоя задача — подготовить его для дальнейшей генерации учебных материалов.

Правила:
1. Удали остаточный технический мусор: субтитровые вставки, случайные повторы, сбои автотранскрибации.
2. Сохраняй педагогически значимые фрагменты: вопросы ученика, ошибки, затруднения, просьбы повторить, методические акценты учителя.
3. Раздели занятие на смысловые блоки.
4. Восстанови формулы в LaTeX, если это возможно. При сомнении ставь: [требуется проверка по исходному условию].
5. Не выдумывай условия задач, данные и ответы. Если фрагмент неясен, пометь: [неясно].
6. Выдели темы занятия, ключевые формулы, разобранные задачи, основные приёмы, ошибки ученика и материал для пособия.
...
- Базовая очистка: `{clean_mode}`
- Смысловая фильтрация для LLM: `{content_filter_mode}`

## Размеры текста

- Сырой текст: {len(raw_text)} символов
- Очищенный текст: {len(cleaned_text)} символов
- Содержательная версия для LLM: {len(content_only_text)} символов
...
1. Открыть `02_clean_*.txt` и проверить качество полной очистки.
2. Открыть `03_content_only_*.txt` и проверить, не удалены ли важные фрагменты.
3. Для генерации пособия использовать `03_content_only_*.txt`, `04_llm_prompt.md` или чанки из папки `03_llm_chunks`.
"""
...
def process_prepared_transcript(
    ...
    content_only_text = build_content_only_transcript(cleaned_text, mode=content_filter_mode)
    text_for_llm = content_only_text if content_filter_mode != "off" else cleaned_text

    chunks = split_text_into_chunks(text_for_llm, max_chars=chunk_size, overlap_chars=chunk_overlap)

    normalized_file = output_dir / "01_normalized.txt"
    clean_file = output_dir / f"02_clean_{clean_mode}.txt"
    content_only_file = output_dir / f"03_content_only_{content_filter_mode}.txt"
    ...
    content_only_file.write_text(content_only_text, encoding="utf-8")

    chunks_dir.mkdir(exist_ok=True)
    for old_chunk in chunks_dir.glob("chunk_*.txt"):
        old_chunk.unlink()
    for index, chunk in enumerate(chunks, start=1):
        (chunks_dir / f"chunk_{index:03}.txt").write_text(chunk, encoding="utf-8")

    prompt_text = create_llm_prompt(
        transcript_text=text_for_llm,
        signals=signals,
    ...
    return {
        ...
        "content_only_file": str(content_only_file),
        ...
        "content_only_chars": len(content_only_text),
    }
...
def ollama_request_json(
    ...
    request = url_request.Request(url=url, data=data, headers=headers, method=method)
    ...
    try:
        ...
        raise RuntimeError(f"Ollama вернул HTTP {exc.code}:\n{error_body}") from exc
    ...
    try:
        ...
        raise RuntimeError(f"Ollama вернула невалидный JSON:\n{raw_body[:1000]}") from exc
...
def ollama_chat(config: OllamaConfig, system_prompt: str, user_prompt: str) -> tuple[str, dict[str, object]]:
    ...
    if not content:
        raise RuntimeError(f"Ollama вернула пустой ответ.\nПолный ответ:\n{json.dumps(response, ensure_ascii=False, indent=2)}")
    return content, response


def build_ollama_chunk_prompt(chunk_text: str, chunk_index: int, total_chunks: int) -> str:
    return f"""Перед тобой очищенный и смыслово отфильтрованный фрагмент транскрипта учебного занятия.
...
def run_ollama_processing_for_lesson(
    ...
    logging.info("Запускаю Ollama-обработку чанков: модель %s, чанков %s", config.model, len(chunk_files))

    for index, chunk_file in enumerate(chunk_files, start=1):
        chunk_text = chunk_file.read_text(encoding="utf-8")
        user_prompt = build_ollama_chunk_prompt(chunk_text, chunk_index=index, total_chunks=len(chunk_files))
        logging.info("Ollama: обрабатываю чанк %s/%s", index, len(chunk_files))
        started = perf_counter()
        content, raw_response = ollama_chat(config=config, system_prompt=OLLAMA_SYSTEM_PROMPT, user_prompt=user_prompt)
    ...
    if config.final_synthesis:
        ...
        if len(final_prompt) > config.final_max_chars:
            warning_text = (
                "# Итоговый протокол автоматически не собран\n\n"
            ...
            started = perf_counter()
            final_content, final_raw_response = ollama_chat(config=config, system_prompt=OLLAMA_SYSTEM_PROMPT, user_prompt=final_prompt)
            elapsed = perf_counter() - started
...
def process_one_audio_file(
    ...
    if ollama_config is not None and ollama_config.enabled:
        ...
        started = perf_counter()
        ollama_result = run_ollama_processing_for_lesson(output_dir=output_dir, preprocess_result=preprocess_result, config=ollama_config)
    ...
    logging.info("Содержательная версия: 03_content_only_%s.txt", content_filter_mode)
...
def collect_audio_files(input_dir: Path, recursive: bool) -> list[Path]:
    ...
    return sorted(file for file in candidates if file.is_file() and file.suffix.lower() in AUDIO_EXTENSIONS)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "предобработкой учебных транскриптов, смысловой фильтрацией и опциональной обработкой через Ollama."
        )
    )
    ...
    parser.add_argument("--cpu-threads", type=int, default=resolve_default_cpu_threads(), help="Количество CPU-потоков для faster-whisper.")
    parser.add_argument("--num-workers", type=int, default=1, help="Количество worker'ов faster-whisper.")
    parser.add_argument("--language", type=str, default=DEFAULT_LANGUAGE, help="Язык речи, например ru, en, de.")
    parser.add_argument("--beam-size", type=int, default=1, help="Ширина beam search. 1 — быстрее, 2–5 — точнее и медленнее.")
    parser.add_argument("--vad-filter", action=argparse.BooleanOptionalAction, default=True, help="Включить VAD-фильтр faster-whisper.")
    parser.add_argument("--vad-min-silence-duration-ms", type=int, default=1000, help="Минимальная длительность тишины для VAD, мс.")
    parser.add_argument("--vad-speech-pad-ms", type=int, default=300, help="Отступ вокруг найденной речи для VAD, мс.")
    parser.add_argument("--word-timestamps", action="store_true", help="Сохранять пословные таймкоды.")
    parser.add_argument("--skip-audio-prepare", action="store_true", help="Передавать исходный файл напрямую в faster-whisper без создания WAV через ffmpeg.")
    parser.add_argument("--audio-filter", type=str, choices=["none", "basic", "loudnorm"], default="basic", help="Фильтр ffmpeg при подготовке WAV.")
    parser.add_argument("--clean-mode", type=str, choices=["conservative", "balanced", "aggressive"], default="balanced", help="Режим базовой очистки транскрипта.")
    parser.add_argument(
        "--content-filter-mode",
        type=str,
        choices=["off", "safe", "medium", "aggressive"],
        default="medium",
        help="Смысловая фильтрация текста перед LLM: off, safe, medium, aggressive.",
    )
    parser.add_argument("--chunk-size", type=int, default=6000, help="Максимальный размер одного чанка для LLM в символах.")
    parser.add_argument("--chunk-overlap", type=int, default=500, help="Перекрытие между чанками в символах.")
    parser.add_argument("--prompt-include-text-limit", type=int, default=30000, help="Лимит вставки текста в 04_llm_prompt.md.")
    parser.add_argument("--initial-prompt", type=str, default=DEFAULT_INITIAL_PROMPT, help="Начальная подсказка для Whisper. Пустая строка отключает.")
    parser.add_argument("--condition-on-previous-text", action="store_true", help="Включить condition_on_previous_text=True.")
    parser.add_argument("--keep-prepared-wav", action="store_true", help="Сохранять подготовленный WAV рядом с результатами.")
    parser.add_argument("--recursive", action="store_true", help="Искать аудиофайлы во вложенных папках.")
    parser.add_argument("--verbose", action="store_true", help="Подробный лог.")
    parser.add_argument("--ollama", action="store_true", help="После очистки транскрипта обработать чанки локальной моделью через Ollama.")
    parser.add_argument("--ollama-url", type=str, default="http://localhost:11434", help="Базовый URL Ollama API.")
    parser.add_argument("--ollama-model", type=str, default="qwen2.5:7b", help="Название локальной модели Ollama.")
    parser.add_argument("--ollama-timeout", type=int, default=900, help="Таймаут одного запроса к Ollama в секундах.")
    parser.add_argument("--ollama-temperature", type=float, default=0.1, help="Температура генерации Ollama.")
    parser.add_argument("--ollama-num-ctx", type=int, default=8192, help="Контекст Ollama.")
    parser.add_argument("--ollama-num-predict", type=int, default=3000, help="Максимальное количество генерируемых токенов.")
    parser.add_argument("--ollama-keep-alive", type=str, default="10m", help="Сколько держать модель Ollama загруженной после запроса.")
    parser.add_argument("--no-ollama-final-synthesis", action="store_true", help="Обработать только отдельные чанки.")
    parser.add_argument("--ollama-final-max-chars", type=int, default=45000, help="Максимальный размер финального prompt.")
...
def main() -> None:
    ...
    audio_files = collect_audio_files(input_dir=input_dir, recursive=args.recursive)
    if not audio_files:
